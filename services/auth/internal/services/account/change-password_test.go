	...
	tests := []struct {
	...
	}{
		{
			...
			mockConfig: func() {
				...
				repo.EXPECT().UpdateOnce(gomock.Any(), gomock.Any()).Return(&entities.RepositoryAccountEntity{ID: "42"}, nil)
			},
		},
		{
			name: "Short password",
		...
		},
		{
			...
			mockConfig: func() {
				repo.EXPECT().FindByID(gomock.Any(), gomock.Any()).Return(
					&entities.RepositoryAccountEntity{ID: "42", Password: passHash},
					nil,
				)
			},
		},
